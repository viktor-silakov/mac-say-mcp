...
function getProjectRoot(): string {
  ...
  // If we're in dist/, go up one level, otherwise use current directory
  return currentDir.endsWith('dist') ? dirname(currentDir) : currentDir;
}

// Global state
const state: ServerState = {
  audioDir: join(getProjectRoot(), 'audio'),
  availableVoices: [],
  defaultVoice: 'Alex',
...
// Tool handlers
async function handleTTSSpeak(params: TTSSpeakParams): Promise<TTSResponse> {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const sayProcess: ChildProcess = spawn('say', sayArgs);
    ...
    return {
      success: false,
      error: `Failed to speak text: ${(error as Error).message}`
    };
  }
}

async function handleTTSSaveAudio(params: TTSSaveAudioParams): Promise<TTSResponse> {
  const {
    text,
    filename,
...
async function handleTTSListVoices(params: TTSListVoicesParams): Promise<TTSResponse> {
  ...
  try {
    ...
    const voicesByLanguage: VoicesByLanguage = voices.reduce((acc, voice) => {
    ...
    return {
      success: false,
      error: `Failed to list voices: ${(error as Error).message}`
    };
  }
}

async function handleTTSVoiceDemo(params: TTSVoiceDemoParams): Promise<TTSResponse> {
  ...
  try {
    ...
    return {
      success: false,
      error: `Failed to play voice demo: ${(error as Error).message}`
    };
  }
}

async function handleTTSSystemInfo(): Promise<TTSResponse> {
  try {
    // Check say command availability
    const sayAvailable = await new Promise<boolean>((resolve) => {
...
function isValidSpeakParams(params: unknown): params is TTSSpeakParams {
  return typeof params === 'object' && params !== null && 
         typeof (params as any).text === 'string';
}

function isValidSaveAudioParams(params: unknown): params is TTSSaveAudioParams {
  return typeof params === 'object' && params !== null &&
         typeof (params as any).text === 'string' &&
         typeof (params as any).filename === 'string';
}

function isValidListVoicesParams(params: unknown): params is TTSListVoicesParams {
  return typeof params === 'object' && params !== null;
}

function isValidVoiceDemoParams(params: unknown): params is TTSVoiceDemoParams {
  return typeof params === 'object' && params !== null &&
         typeof (params as any).voice === 'string';
}
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      case "tts_speak":
        if (!isValidSpeakParams(params)) {
          throw new Error('Invalid parameters for tts_speak: text is required');
        }
        result = await handleTTSSpeak(params);
        break;
      case "tts_save_audio":
        if (!isValidSaveAudioParams(params)) {
          throw new Error('Invalid parameters for tts_save_audio: text and filename are required');
        }
        result = await handleTTSSaveAudio(params);
        break;
      case "tts_list_voices":
        if (!isValidListVoicesParams(params)) {
          throw new Error('Invalid parameters for tts_list_voices');
        }
        result = await handleTTSListVoices(params);
        break;
      case "tts_voice_demo":
        if (!isValidVoiceDemoParams(params)) {
          throw new Error('Invalid parameters for tts_voice_demo: voice is required');
        }
